...
func TestLoginSupersedesPriorRunForSameHelperWithoutDetachingRoute(t *testing.T) {
	now := time.Unix(100, 0).UTC()
	journal, _ := operation.NewJournal(8)
	...
	if err := adapter.AuthorizeProxyRun(first.RunID.Value); err == nil {
		t.Fatal("superseded run remained authorized")
	}
	adapter.Close(first.RunID.Value)
	if _, ok := registry.Get("rte_1"); !ok {
		t.Fatal("superseded close detached replacement route")
	}
