...
jq -e --arg repo "$repo" '.version == "2.0.0" and (.consumers[$repo] | type == "object") and (.unsupported.mutation_allowed == false) and (.unsupported.error_code == "protocol_incompatible")' "$contracts/compatibility/matrix.json" >/dev/null || { echo "$repo contracts: invalid compatibility claim" >&2; exit 1; }
