...
var fixtureNodePattern = regexp.MustCompile(`^[a-z][a-z0-9_-]{0,127}$`)

func main() {
	var directory, protocol, nodeID string
	var generation, revision uint64
	var tcpPort, quicPort int
	...
	flag.StringVar(&nodeID, "node-id", "edge-vps-01", "assigned edge node")
	flag.IntVar(&tcpPort, "tcp-port", 26022, "frps TCP connector port")
	flag.IntVar(&quicPort, "quic-port", 26023, "frps QUIC connector port")
	...
	flag.Parse()
	if directory == "" || (protocol != "tcp" && protocol != "quic") || !fixtureNodePattern.MatchString(nodeID) || tcpPort < 1 || tcpPort > 65535 || quicPort < 1 || quicPort > 65535 || tcpPort == quicPort || generation == 0 || revision == 0 {
		fatal("invalid fixture arguments")
	}
	if err := generateFixture(directory, fixtureOptions{Protocol: protocol, NodeID: nodeID, TCPPort: tcpPort, QUICPort: quicPort, Generation: generation, Revision: revision, Revoked: revoked, IncludeRoute: includeRoute, LoseUsageAck: loseUsageAck}, time.Now().UTC()); err != nil {
		fatal(err.Error())
	}
}
...
func generateFixture(directory string, options fixtureOptions, now time.Time) error {
	if options.NodeID == "" {
		options.NodeID = "edge-vps-01"
	}
	if options.TCPPort == 0 {
		options.TCPPort = 26022
	}
	if options.QUICPort == 0 {
		options.QUICPort = 26023
	}
	if !fixtureNodePattern.MatchString(options.NodeID) || options.TCPPort < 1 || options.TCPPort > 65535 || options.QUICPort < 1 || options.QUICPort > 65535 || options.TCPPort == options.QUICPort {
		return fmt.Errorf("invalid fixture node or ports")
	}
	...
	handoff, _ := json.Marshal(map[string]any{"operation_id": fmt.Sprintf("op_phase3_admit_%04d", options.Generation), "credential": credential, "environment_id": "env_phase3_01", "helper_id": "helper_phase3_01", "connector_generation": options.Generation, "edge_pool": "default", "edge_node_id": options.NodeID, "routes": []any{route}})
	serverPort := options.TCPPort
	if options.Protocol == "quic" {
		serverPort = options.QUICPort
	}
